...
export default function HomeClient() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (demoRole === 'admin') return // Admin sees no buyer portal

      if (demoRole === 'buyer') {
        setProfile({ full_name: 'James Wilson', role: 'buyer', status: 'approved', company_id: null })
        setCompany({ name: 'Wilson Interiors Ltd' })
        return
      }
      ...
      if (p) {
        if (p.role === 'admin') return // Real admin - no buyer portal
        setProfile(p)
        if (p.company_id) {
