                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  {canSeePrice && (
                    <button
                      ...
                      className={`text-[10px] tracking-wider uppercase px-2.5 py-1 rounded transition-all ${
                        basketItems.some(i => i.productId === product.id)
                          ? 'bg-emerald-600/20 text-emerald-400 border border-emerald-600/30'
                          : 'bg-[var(--gold)]/10 text-[var(--gold)] border border-[var(--gold)]/20 hover:bg-[var(--gold)]/20'
                      }`}
                    >
                      {basketItems.some(i => i.productId === product.id) ? '✓ In Basket' : '+ Quote'}
                    </button>
                  )}
                </div>
              </div>
