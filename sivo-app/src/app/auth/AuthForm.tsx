...
const DEMO_ACCOUNTS = [
  {
    ...
    role: 'admin',
    color: '#c9a96e',
    bg: 'rgba(201,169,110,.08)',
    border: 'rgba(201,169,110,.3)',
  },
  {
    label: 'Buyer',
    icon: '🛒',
  ...
  },
  {
    ...
    icon: '🏭',
    sub: 'Raj Patel',
    company: 'GHP Manufacturing',
    desc: 'Production partner view',
...
export default function AuthForm() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data: companyData, error: companyError } = await supabase
      .from('companies')
      .insert({ name: regCompany, vat_number: regVat || null, website: regWebsite || null, business_type: regBusinessType, annual_spend: regSpend || null, status: 'pending' })
      .select().single()
    ...
    <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 16px' }}>
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        <div style={{
          ...
          background: 'linear-gradient(135deg, rgba(201,169,110,.05), transparent)',
          border: '1px solid rgba(201,169,110,.2)',
          borderRadius: 12,
        }}>
          <div style={{ fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', color: 'var(--gold)', textAlign: 'center', marginBottom: 16 }}>
            ⚡ Quick Demo Login
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 14 }}>
            {DEMO_ACCOUNTS.map(acc => (
              <button
                ...
                style={{
                  background: demoLoading === acc.email ? acc.bg : 'transparent',
                  border: `1px solid ${demoLoading === acc.email ? acc.color : 'var(--border)'}`,
                  borderRadius: 10,
                  padding: '14px 8px',
              ...
              >
                <div style={{ fontSize: 26, lineHeight: 1 }}>
                  {demoLoading === acc.email ? '⏳' : acc.icon}
                </div>
                <div style={{ fontSize: 11, fontWeight: 700, color: acc.color, letterSpacing: .5 }}>
                  {demoLoading === acc.email ? 'Signing in...' : acc.label}
                </div>
                <div style={{ fontSize: 9, color: 'var(--muted)', lineHeight: 1.4 }}>
                  {acc.sub}<br />{acc.company}
                </div>
                <div style={{ fontSize: 8, color: acc.color, opacity: .7, marginTop: 2 }}>
                  {acc.desc}
                </div>
              </button>
            ))}
        ...
        {tab === 'login' && (
          <form onSubmit={handleLogin}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="email" className="input-field" placeholder="your@company.co.uk" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} required />
            </div>
            <div style={{ marginBottom: 16 }}>
              <label className="input-label">Password</label>
              <input type="password" className="input-field" placeholder="Enter password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} required />
            </div>
            <button type="submit" disabled={loading} className="btn-gold" style={{ width: '100%', fontSize: 12, letterSpacing: '1px', textTransform: 'uppercase' }}>
        ...
        {tab === 'register' && !success && (
          <form onSubmit={handleRegister}>
            <div className="card" style={{ padding: 14, marginBottom: 16, background: 'var(--surface)', fontSize: 12, color: 'var(--muted)', lineHeight: 1.6 }}>
